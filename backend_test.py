...
class MatrimonyAPITester:
    ...
    def make_user_premium(self):
        """Manually make the test user premium in the local DB for testing"""
        try:
            from pymongo import MongoClient
            client = MongoClient("mongodb://localhost:27017")
            db = client["satnami_db"]
            db.users.update_one(
            ...
            # Re-fetch auth/me to refresh cookies/session
            self.session.get(f"{self.base_url}/auth/me")
            print("🚀 Test user upgraded to premium in DB")
            return True
        except Exception as e:
            print(f"⚠️ Could not manually upgrade user to premium: {e}")
            return False
    ...
    def test_messaging(self):
        ...
        # Upgrade test user to premium so messaging succeeds
        self.make_user_premium()
